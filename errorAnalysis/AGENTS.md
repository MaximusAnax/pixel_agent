...
- Tests: `pytest`.
- Project-state loop lives in `ops/` (weekly report, meeting transcription, state
  synthesis). The `Live project state` block above is auto-generated by
  `ops/synthesize_state.py` — never edit it by hand. Drive the loop with the
  `project-state-sync` skill; full design in `docs/project_state_automation.md`.
