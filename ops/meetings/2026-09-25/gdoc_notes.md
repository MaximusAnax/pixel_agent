...
> Pulled from Google Doc **Pixel Agent Meeting Notes (Abdoul + Raghav + Amaad + Matt)** on 2026-09-25.
...
Updates
https://github.com/raghav3003/pixel_agent/blob/goldjudge-stage/docs/research/2026-09-24-project-summary.md 

New idea
For CUA, one problem we’ve seen is that agents don’t always know where to, say, find the setting they are supposed to change. So dispatch multiple subagents to explore different paths (e.g. each one clicks a different tab)
Further, if the exploration is stored in a vector database (or text database), then the next time the model needs to tackle a similar task (or a task within the same UI) it makes the future task easier
Could we use multiple Chrome instances to make this more efficient and avoid the issue of side-effects?
This is analogous to what coding agents do when tasked with learning about a large codebase (e.g. dispatch 3 subagents to read 3 different parts of the codebase and report back), but materially different in that reading a code base is free of side effects, but interacting with a CUA is not
Sort of a test-time compute tradeoff
Downside: maybe the resulting agent is now more expensive or slower than if it didn’t do this exploration
Bring someone new into the project?
Yes, let’s do it.

Ten (10) Cray HPE XD670 nodes with:
2048GB RAM, DDR-4800
4 NVMe SSds (7.68TB total)
8 NVIDIA H100-80GB GPUs
2 Intel Xeon “Sapphire Rapids” 8470 CPUs
52 cores, 2.0-3.8 GHz
Three (3) HPE DL380aG12 nodes with:
1024GB RAM
8 EDSFF NVMe drives (3.2TB )
8 NVIDIA L40S-48GB GPUs
2 Intel Xeon 6740E CPUs 
96 cores, 2.4GHz
